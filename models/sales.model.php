...
class ModelSales{
  ...
  //Adding total this month sales（今月だけ）
  static public function mdlAddingTotalThisMonthSales($table){
    $stmt = Connection::connect()->prepare("SELECT SUM(totalPrice) as total_this_month FROM $table WHERE sell_date between date_format(now(), '%Y-%m-01') and last_day(now());");
    $stmt -> execute();
    return $stmt -> fetch();
    $stmt -> close();
    $stmt = null;
  ...
  static public function mdlAddingTotalThisMonthProductsSales($table){
    $stmt = Connection::connect()->prepare("SELECT SUM(product_sales) as total FROM $table WHERE sell_date between date_format(now(), '%Y-%m-01') and last_day(now());");
    $stmt -> execute();
    return $stmt -> fetch();
    $stmt -> close();
    $stmt = null;
